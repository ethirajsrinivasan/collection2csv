...
class Collection2csvController < ApplicationController
  ...
  def initialize_with_params
    ...
    @column_names = params[:column_names].presence || @klass.column_names
    @associations = params[:associations].presence || {}
    @association_class_names = {}
    @association_column_names = []
  end

  def validate
    validate_columns if params[:column_names].present?
    validate_associations if params[:associations].present?
  end

  def download
    @collections = @klass.includes(@associations.try(:keys)).find(params[:ids])
    send_data to_csv(@collections,@column_names,@associations), filename: get_filename
  end
